...
export const resolveImageUrl = (url?: string) => {
  ...
  if (match && match[1]) {
    ...
    // Attempt to decode Base64URL (New format)
    try {
        // Basic check to see if it looks like base64 (alphanumeric + - _)
        if (/^[a-zA-Z0-9\-_]+$/.test(key) && !key.includes('%')) {
             const decoded = atob(key.replace(/-/g, '+').replace(/_/g, '/'));
             // Validation: The decoded key should normally be a clean string like "uuid/uuid.png"
             // If it contains garbage, it might not be base64.
             if (decoded.includes('/') || decoded.includes('.')) {
                 key = decoded;
             }
        }
    } catch (e) {
        // Not valid base64, likely old format
    }

    // Fallback: If it contains % or looks like URI component (Old format), decode it
    if (key.includes('%')) {
        key = decodeURIComponent(key);
    }

    // If R2 Public Domain is set, use it
    if (domain) {
